...
assert.match(productTable, /그룹에서 분리한 상품/)
assert.match(productTable, /isManuallyUngrouped/)
assert.match(productTable, /scrollIntoView/)
assert.match(productTable, /getNaverSellingProductUrl/)
assert.doesNotMatch(productTable, /smartstore\.naver\.com\/beiko\/search\?q=/)
assert.match(productTable, /네이버 스마트스토어 판매 상품 보기/)
