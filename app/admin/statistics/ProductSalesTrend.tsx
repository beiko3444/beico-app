...
export default function ProductSalesTrend({
  ...
  const chartWidth = Math.max(720, points.length * 74)
  ...
  const barWidth = Math.max(5, Math.min(30, step * 0.58))
  const linePoints = points.map((point, index) => {
    const x = points.length === 1 ? plotStart + plotWidth / 2 : plotStart + step * (index + 0.5)
  ...
  return (
    <section className="border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px] font-medium text-slate-500">
            조회 기간({rangeText})의 네이버 일별 통계 기준 {bucketLabel} 매출과 판매수량을 보여줍니다.
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        ...
        <Metric label="피크" value={peak ? `${peak.label} · ${formatWon(peak.total)}` : '-'} />
      </div>
          ...
          <div className="relative h-full" style={{ width: `${chartWidth}px` }}>
            <svg viewBox="0 0 600 190" className="h-[205px] w-full overflow-visible" role="img" aria-label={`${selectedName} 판매추세`}>
              ...
              {points.map((point, index) => {
                const x = plotStart + step * index + (step - barWidth) / 2
                const height = Math.max(point.total > 0 ? 6 : 0, (point.total / maxSales) * 124)
                return (
                  <g key={point.periodKey}>
                    <rect x={x} y={166 - height} width={barWidth} height={height} rx={Math.min(7, barWidth / 2)} fill="#CBD5E1" />
            ...
            <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${Math.max(points.length, 1)}, minmax(68px, 1fr))` }}>
              {points.map((point) => (
