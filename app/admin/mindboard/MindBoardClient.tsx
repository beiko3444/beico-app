...
export default function MindBoardClient() {
    ...
    // Stored in board/world coordinates. Render converts it back to screen coordinates.
    ...
    }, [isPanning, dragItems, resizeItem, resizeGroup, selectionBox, scale, isMinimapDragging, getWorldCoords])
    ...
    const finalizeInteraction = useCallback(() => {
        ...
        if (selectionBox) {
            const wx1 = Math.min(selectionBox.startX, selectionBox.currentX)
            const wx2 = Math.max(selectionBox.startX, selectionBox.currentX)
            const wy1 = Math.min(selectionBox.startY, selectionBox.currentY)
            const wy2 = Math.max(selectionBox.startY, selectionBox.currentY)
        ...
        autoPanVelocity.current = { x: 0, y: 0 }
    }, [dragItems, selectionBox, items, selectedIds, updateAutoPan])
