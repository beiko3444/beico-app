...
exports.scrapeLotteCardUsage = scrapeLotteCardUsage;
...
async function connectToPage(wsUrl) {
    ...
    }), 8000, 'Browser debug WebSocket connection');
}
async function getTargets(debugUrl) {
...
async function findLotteTarget(debugUrl) {
    const targets = await getTargets(debugUrl);
    const pages = targets.filter((target) => target.type === 'page' && target.webSocketDebuggerUrl);
    const isLoginPage = (target) => /login|\uB85C\uADF8\uC778/i.test(target.title || '');
    ...
    if (!target?.webSocketDebuggerUrl) {
        throw new Error('LotteCard browser session was not found. Open Whale/Chrome with --remote-debugging-port=9222 and log in first.');
    }
    return target;
}
...
function normalizeRow(row) {
    ...
    const isCanceled = hasMeaningfulValue(raw.aprCanRc) ||
    ...
    const useDT = compact(raw.aprDtti).slice(0, 14) ||
        (usedAt
...
async function navigateToUsagePage(page) {
    ...
    await waitFor(page, async () => {
        const state = await page.evaluate(`(() => ({ href: location.href, ready: document.readyState }))()`, false);
        return state.href.includes('LPMCDAA_V100') && state.ready !== 'loading';
    }, 'LotteCard usage page');
}
async function waitFor(page, check, label, timeoutMs = 20000) {
    ...
    const state = await page.evaluate(`(() => ({ href: location.href, title: document.title }))()`, false);
    throw new Error(`${label} did not become ready. Current page: ${state.title} ${state.href}`);
}
async function searchPeriod(page, startDate, endDate) {
...
async function loadAllRows(page) {
    ...
    for (; clicks < 100; clicks += 1) {
      ...
      return {
        ...
        staleClicks += 1;
        if (staleClicks >= 2 || !after.canClick)
            break;
    }
...
async function scrapeLotteCardUsage(options) {
