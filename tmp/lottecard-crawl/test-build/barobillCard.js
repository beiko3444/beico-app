...
exports.getCardEx2 = getCardEx2;
exports.refreshCard = refreshCard;
exports.getPeriodCardApprovalLog = getPeriodCardApprovalLog;
exports.getPeriodCardLogEx3 = getPeriodCardLogEx3;
exports.fetchCardUsageByPeriod = fetchCardUsageByPeriod;
...
function assertCardConfig() {
    ...
    if (!cfg.CERTKEY)
        throw new Error('BAROBILL_CERTKEY is not configured.');
    if (!cfg.CORP_NUM)
        throw new Error('BAROBILL_CORP_NUM is not configured.');
    if (!cfg.ID)
        throw new Error('BAROBILL_CARD_ID or BAROBILL_CONTACT_ID is not configured.');
    return cfg;
}
...
async function callCardSoap(action, bodyXml) {
...
function normalizeCardApprovalType(rawType, amountHints) {
...
function buildStoredUseKey(useKeyRaw, fallbackUseKey, normalizedType, amountHints) {
    const base = (useKeyRaw || fallbackUseKey || '').trim();
    if (!base)
        return fallbackUseKey;
    ...
    if (!shouldSuffixCancel)
        return base;
    if (base.endsWith(':CANCEL'))
        return base;
    return `${base}:CANCEL`;
}
function resolveBarobillLogAmount(log) {
    if (typeof log.totalAmount === 'number' && log.totalAmount > 0)
        return log.totalAmount;
    if (typeof log.approvalAmount === 'number' && log.approvalAmount > 0)
        return log.approvalAmount;
...
function parseCardApprovalLog(block) {
    ...
    const approvalAmount = toInt(extractTag(block, 'ApprovalAmount') ||
    ...
    const summedAmount = typeof amount === 'number' || typeof tax === 'number' || typeof serviceCharge === 'number'
        ? (amount || 0) + (tax || 0) + (serviceCharge || 0)
        : null;
    const totalAmount = (typeof rawTotalAmount === 'number' && rawTotalAmount > 0)
        ? rawTotalAmount
    ...
    const useKey = buildStoredUseKey(useKeyRaw, fallbackUseKey, normalizedApprovalType, [totalAmount, approvalAmount, amount]);
    return {
        corpNum,
        cardNum,
        useKey,
...
function parseLegacyCardLogEx3(block) {
    const corpNum = decodeXml(extractTag(block, 'CorpNum') || '');
    const cardNum = decodeXml(extractTag(block, 'CardNum') || '');
    const useDT = decodeXml(extractTag(block, 'UseDT') || '');
...
function parsePagedCardLogResult(xml, options) {
...
async function getCardEx2(availableOnly = true) {
...
async function refreshCard(cardNum) {
    const cfg = assertCardConfig();
...
async function getPeriodCardApprovalLog(params) {
    const cfg = assertCardConfig();
...
async function getPeriodCardLogEx3(params) {
    const cfg = assertCardConfig();
    const startDate = toYmd(params.startDate);
    const endDate = toYmd(params.endDate);
    const countPerPage = Math.max(1, Math.min(1000, params.countPerPage || 500));
...
function mergeLegacyLogAmounts(currentLogs, legacyLogs) {
    ...
    return currentLogs.map((log) => {
        ...
        if (!legacy)
            return log;
        if (resolveBarobillLogAmount(log) > 0)
            return log;
        return {
            ...log,
...
async function fetchCardUsageByPeriod(params) {
    ...
    for (const cardNum of targetCards) {
        ...
        const needsLegacyFallback = approvalLogs.length === 0 ||
